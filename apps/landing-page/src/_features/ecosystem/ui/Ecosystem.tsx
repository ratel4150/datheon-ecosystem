...
const gridVariants = {
  hidden: {},
  visible: { transition: { staggerChildren: 0 } }, // el delay real va por nodo vía `custom`, no por stagger genérico
};
  ...
  return (
    ...
    >
      ...
      <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1 }}>
        <EcosystemHeader kicker={t.kicker} title={t.title} subtitle={t.subtitle} textColor={T.text} textMuteColor={T.textMute} accentColor={T.accent} glow={T.glow} />

        <EcosystemCoreCard accent={T.accent} accentDk={T.accentDk} accentBg={T.accentBg} glow={T.glow} surface={T.surface} text={T.text} />
        ...
        <Box sx={{ textAlign: 'center', mt: { xs: 4, md: 5 } }}>
          <EcosystemCta label={t.cta} accentColor={T.accent} accentBg={T.accentBg} delay={ctaDelay} />
        </Box>
      </Container>
    </Box>
  );
