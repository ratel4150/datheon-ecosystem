...
// Diferencia clave respecto a la versión anterior en landing-page: ya NO
// llama useTheme() por dentro — recibe los tokens T como prop, para no
// acoplar el paquete compartido a la implementación de tema de una app
// específica. Cada app consumidora le pasa sus propios tokens.
import { useState } from 'react';
import { Box, Container, Stack, Typography, Button, Switch, GlobalStyles } from '@mui/material';
import { motion, AnimatePresence } from 'framer-motion';
import type { ConsentChoices } from '../lib';
import { useCookieConsent } from '../model';

export interface CookieConsentTokens {
  surface: string;
  text: string;
  textMid: string;
  textMute: string;
...
export function CookieBanner({ t, T }: Props) {
  ...
  return (
    <AnimatePresence>
      <Box
        ...
        sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 1400, bgcolor: T.surface, borderTop: `1px solid ${T.border}`, boxShadow: '0 -4px 24px rgba(0,0,0,0.12)' }}
      >
          ...
          {!expanded ? (
            <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ alignItems: { xs: 'stretch', sm: 'center' }, justifyContent: 'space-between', gap: 2 }}>
              ...
              <Stack direction="row" sx={{ gap: 1, flexShrink: 0, flexWrap: 'wrap' }}>
                <Button onClick={openSettings} className="cc-focus" sx={{ fontSize: '0.75rem', color: T.textMid, textTransform: 'none', '&:hover': { color: T.accent, bgcolor: 'transparent' } }}>
                  {t.customize}
                </Button>
                <Button onClick={rejectAll} className="cc-focus" sx={{ fontSize: '0.78rem', color: T.text, textTransform: 'none', border: `1px solid ${T.border}`, borderRadius: '8px', px: 2, '&:hover': { borderColor: T.accent } }}>
                  {t.rejectAll}
                </Button>
                <Button onClick={acceptAll} variant="contained" className="cc-focus" sx={{ bgcolor: T.accent, color: '#fff', fontWeight: 700, fontSize: '0.78rem', px: 2.25, borderRadius: '8px', textTransform: 'none', '&:hover': { bgcolor: T.accentDk } }}>
                  {t.acceptAll}
                </Button>
              </Stack>
            </Stack>
            ...
            <Stack spacing={2}>
              ...
              <Stack direction="row" sx={{ justifyContent: 'flex-end', gap: 1, pt: 1 }}>
                <Button onClick={handleSave} variant="contained" className="cc-focus" sx={{ bgcolor: T.accent, color: '#fff', fontWeight: 700, fontSize: '0.78rem', px: 2.5, borderRadius: '8px', textTransform: 'none', '&:hover': { bgcolor: T.accentDk } }}>
                  {t.save}
                </Button>
              </Stack>
            </Stack>
